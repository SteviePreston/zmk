...
- Prospector module pulled from `carrefinho/prospector-zmk-module:main` via `west.yml` (modeled on beekeeb's reference config). ZMK itself is pinned to commit `abb64ba` (the last pre-Zephyr-4.1 commit, Dec 7 2025) because MechboardsLTD/zmk-module's `corne-min` board uses the HWMv1 layout (`boards/arm/corne_min/`) and is incompatible with Zephyr 4.1's HWMv2. The pin also keeps the Corne `nice_view_adapter` shield resolving correctly with `BOARD=nice_nano` (which 4.1 requires to be qualified to `nice_nano_nrf52840_zmk`). If MechboardsLTD ever ships an HWMv2 Corne Min, ZMK can move back to `main` and the prospector revision should move to `core/zephyr-4-1`.
